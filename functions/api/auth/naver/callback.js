...
export async function onRequestGet(context) {
  ...
  try {
    ...
    if (!clientId || !clientSecret) {
      return Response.redirect(`${new URL(request.url).origin}/index.html?error=네이버 로그인이 설정되지 않았습니다.`, 302);
    }
    ...
    // 2. 사용자가 네이버 로그인을 취소한 경우
    if (error) {
      return Response.redirect(`${new URL(request.url).origin}/index.html?error=네이버 로그인이 취소되었습니다.`, 302);
    }
    ...
    if (!tokenResponse.ok) {
      ...
      return Response.redirect(`${new URL(request.url).origin}/index.html?error=네이버 인증에 실패했습니다.`, 302);
    }
    ...
    if (naverUserData.resultcode !== '00' || !naverUserData.response) {
      return Response.redirect(`${new URL(request.url).origin}/index.html?error=사용자 정보를 가져오지 못했습니다.`, 302);
    }
    ...
    const user = await createOrGetSocialUser(env['nam-web-app-db'], {
      ...
      name: naverUser.name || naverUser.nickname || naverUser.email?.split('@')[0] || '네이버 사용자',
